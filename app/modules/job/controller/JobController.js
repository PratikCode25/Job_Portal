...
const { jobValidationSchema } = require("../validation/jobValidationSchema");
...
class JobController {
    ...
    async getPublicJobDetailPage(req, res) {
        try {
            const jobId = req.params.id;
            const job = await jobRepositories.getDetailJobById(jobId);
            if (!job) {
                return res.redirect('/jobs/page');
            }
    ...
    async getCandidateJobDetailPage(req, res) {
        try {
            const jobId = req.params.id;
            const userId = req.user._id;
            ...
            return res.render('candidate/job-detail', { job, postedTimeAgo, title: 'Job Detail' });
